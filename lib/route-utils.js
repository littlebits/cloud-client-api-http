...
function route_functionify(route){
  return function(opts, cb){
    ...
    r.end(function(err, response){
      ...
      if (is_ok_status_code(response.statusCode)) return cb(null, response.body);
      cb(http_err_to_js_err(response.body));
    });
  };
}
...
function http_err_to_js_err(res_body){
  if (res_body && res_body.message) return new Error(res_body.message);
  return new Error('HTTP Request returned some sort of error: ' + res_body);
}

function is_ok_status_code(status_code){
